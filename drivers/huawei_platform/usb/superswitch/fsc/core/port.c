...
void FUSB3601_InitializeVars(struct Port *port, FSC_U8 id, FSC_U8 i2c_addr)
{
	FSC_U32 i = 0;
	  ...
	  port->caps_source_[0].FPDOSupply.MaxCurrent = 100;
	  ...
	  port->display_port_data_.DpModeEntryMask.word = 0;
...
void FUSB3601_InitializePort(struct Port *port)
{
  ...
  port->registers_.FMControl1.byte = 0x00;
  port->registers_.FMControl2.byte = 0x00;
  port->registers_.FMControl3.byte = 0x00;
  port->registers_.FMControl4.byte = 0x20;
  ...
  /* Setup the WatchDog */
#if 0
  port->registers_.MUSControl2.WD_EN = 1;
  WriteRegister(port, regMUS_CONTROL_2);

  port->watchdogenabled_ = TRUE;
...
//  SetTimeInterrupt(60000000); /* Set for every minute */
}

/* Register Update Function - Single register read */
FSC_BOOL FUSB3601_ReadRegister(struct Port *port, enum RegAddress regaddress)
{
  return FUSB3601_platform_i2c_read(port->port_id_, (FSC_U8)regaddress, 1,
                 FUSB3601_AddressToRegister(&port->registers_, regaddress));
}
...
{
  return FUSB3601_platform_i2c_read(port->i2c_addr_, (FSC_U8)regaddr, cnt,
                           FUSB3601_AddressToRegister(&port->registers_, regaddr));
}
...
void FUSB3601_ReadRxRegisters(struct Port *port, FSC_U8 numbytes)
{
  ...
  FUSB3601_platform_i2c_read(port->port_id_, regRXDATA_00, numbytes,
                    port->registers_.RxData);
}

void FUSB3601_WriteRegister(struct Port *port, enum RegAddress regaddress)
{
  FUSB3601_platform_i2c_write(port->port_id_, (FSC_U8)regaddress, 1,
           FUSB3601_AddressToRegister(&port->registers_, regaddress));
}

void FUSB3601_WriteRegisters(struct Port *port, enum RegAddress regaddress, FSC_U8 cnt)
{
  FUSB3601_platform_i2c_write(port->port_id_, (FSC_U8)regaddress, cnt,
           FUSB3601_AddressToRegister(&port->registers_, regaddress));
}
...
void FUSB3601_WriteTxRegisters(struct Port *port, FSC_U8 numbytes)
{
  ...
  FUSB3601_platform_i2c_write(port->port_id_, regTXDATA_00, numbytes,
                     port->registers_.TxData);
}
...
//	  data = 0x01;
//	  platform_i2c_write(port->port_id_, 0xF0, 1, &data);
...
FSC_BOOL FUSB3601_IsVbusVSafe5V(struct Port *port)
{
  ...
  return ((measurement > FSC_VSAFE5V_L) &&
          (measurement < FSC_VSAFE5V_H)) ? TRUE : FALSE;
}
...
void FUSB3601_PDEnable(struct Port *port, FSC_BOOL is_source)
{
  ...
  if (port->pd_enabled_ == TRUE) {
	  ...
	  //WriteRegister(port, regPD_RESET);
      ...
      FUSB3601_ClearInterrupt(port, regALERTL, MSK_I_TXSUCC | MSK_I_TXDISC |
      ...
      if(port->policy_state_ == peSourceStartup) FUSB3601_PolicySourceStartup(port);
      else FUSB3601_PolicySinkStartup(port);
  }
}
...
//  SopType sop = 0;
  ...
  if (port->policy_is_source_) {
    port->registers_.RxDetect.EN_SOP1 = 1;
