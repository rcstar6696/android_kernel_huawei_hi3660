...
static void hi3660_dplus_pulldown(struct hisi_dwc3_device *hisi_dwc)
{
...
static void hi3660_dplus_pullup(struct hisi_dwc3_device *hisi_dwc)
{
	void __iomem *base = hisi_dwc->otg_bc_reg_base;
	volatile u32 reg;
...
static void hi3660_check_voltage(struct hisi_dwc3_device *hisi_dwc)
{
	usb_dbg("+\n");

	if (hisi_dwc->check_voltage) {
		/*first dplus pulldown*/
		hi3660_dplus_pulldown(hisi_dwc);
		/*second call charger's API to check voltage */
		water_detect();
		/*third dplus pullup*/
		hi3660_dplus_pullup(hisi_dwc);
	}
...
static struct usb3_phy_ops hi3660_phy_ops = {
	...
	.check_voltage = hi3660_check_voltage,
};
