              ...
              unsigned long args)
{
    ...
    struct coredump_user_para coredump_para;
    int ret;
    ...
    case LOG_WR_OFFSET:
     ...
     case LOG_COREDUMP_START:
        if ((ret = copy_from_user(&coredump_para, (void __user *)args, (int)sizeof(coredump_para))) < 0) {
             pr_err("[%s] Failed : copy_from_user.%d\n", __func__, ret);
             return -EFAULT;
        }
        pr_err("[%s][coredump]  coredump_addr[0x%x] share_para[%pK]\n",__func__,coredump_para.coredump_addr,dev->share_para);
        dev->share_para->coredump_addr = coredump_para.coredump_addr;
     ...
     case LOG_COREDUMP_WRITE:
        ...
        }else if(dev->share_para->exc_flag & CORE_DUMP_EXCEPTION)
        {
            dev->share_para->exc_flag = 0;//clear dump exception
            pr_err("[%s][%d][coredump]  clear dump exception  bit success\n",__func__,__LINE__);
        }
        break;
    default:
