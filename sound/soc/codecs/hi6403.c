...
static void hi6403_init_chip(struct snd_soc_codec *codec)
{
	...
	if (priv->board_config.hs_det_trigger_mode == SCHMIT_MODE) {
		/* set hsd -> schmitt */
		hi64xx_update_bits(codec, HI6403_ANALOG_REG98,
...
static void hi6403_get_board_hsdetmode(struct device_node *node, struct hi6403_board_cfg *board_cfg)
{
	unsigned int val = 0;

	board_cfg->hs_det_trigger_mode = CMP_MODE;
	if (!of_property_read_u32(node, "hisilicon,hs_det_trigger", &val)) {
		if (val) {
			board_cfg->hs_det_trigger_mode = SCHMIT_MODE;
			pr_info("%s : schmit trigger mode\n", __FUNCTION__);
		}
	}
}
...
static void hi6403_get_board_cfg(struct device_node *node, struct hi6403_board_cfg *board_cfg)
{
	...
	hi6403_get_board_hsdetmode(node, board_cfg);
#ifdef CONFIG_HAC_SUPPORT
